            ...
            case 2:
                ...
                if (choice == 1) {
                    ...
                    if (num > 0) // Not gonna be that easy :P 
                    {
                        ...
                        if (tot <= bounty_points && tot >= 0) 
                        {
                            bounty_points -= tot;
                            printf ("Total bounty points after fight are: %d\n", bounty_points);
                        }
                        else if (tot < 0) {
                            printf ("Die, Evil Hacker!\n");
                        }
                        else if (tot > bounty_points) {
                    ...
                    printf ("It will require 100000 bounty points to know the secret\n");
                    printf ("Type 1 to know it\n");
                    int reply;
                    cin >> reply;
                    if (reply == 1) {
                        if (override || bounty_points > 100000) {
                            ...
                            char notallowed4[5] = "echo";
                            char notallowed5[4] = "rev";
                            char notallowed6[] = ";";
                            char notallowed7[] = "|";
                            char notallowed8[] =  "&";
                            char notallowed9[] = "\n";
                            char notallowed10[] = "`";
                            if (checkPresent(notallowed1, cmd) == -1 && checkPresent(notallowed2, cmd) == -1 && checkPresent(notallowed3, cmd) == -1 &&  checkPresent(notallowed4, cmd) == -1 &&  checkPresent(notallowed5, cmd) == -1 &&  checkPresent(notallowed6, cmd) == -1 &&  checkPresent(notallowed7, cmd) == -1 &&  checkPresent(notallowed8, cmd) == -1 &&  checkPresent(notallowed9, cmd) == -1 &&  checkPresent(notallowed10, cmd) == -1) 
                            {
                                FILE *f;
                                char line[100];
                                f = popen(cmd , "r");
